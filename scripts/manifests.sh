...
function prepare_nfs() {
    ...
    # Generate nfs-sno.yaml with storage class and node affinity
    update_file_multi_replace \
        "${MANIFESTS_DIR}/nfs/nfs-sno.yaml" \
        "${GENERATED_DIR}/nfs-sno.yaml" \
        "<STORAGECLASS_NAME>" "${ETCD_STORAGE_CLASS}"
    
    # Replace node affinity placeholder
    sed -i "" "s|<NODE_AFFINITY>|${node_affinity}|g" "${GENERATED_DIR}/nfs-sno.yaml"
