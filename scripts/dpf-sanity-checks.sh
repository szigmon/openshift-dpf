...
#### debug: remove later:
echo -e "\n######## Debug:  Total testcases executed: '${total_testcases_executed}' \nFailed testcase count so far: '${failed_testcase_count}' \nTestcase Results summary is:\n${test_results_summary}"
...
for i in "${!dpu_workers[@]}"; do
  ## echo "Finding doca hbn pod ip address for dpu_workers array index $i '${dpu_workers[$i]}' for doca-hbn pod name '${doca_hbn_worker_pods[$i]}'"
  ...
  ## echo -e "doca-hbn pod ip address for worker '${dpu_workers[$i]}' found was: '${doca_hbn_worker_pod_ip[$i]}'"
  echo -e "doca-hbn pod '${doca_hbn_worker_pods[$i]}' on DPU worker node '${dpu_workers[$i]}' has ip address:  '${doca_hbn_worker_pod_ip[$i]}'"
done
