...
install-dpf-kubectl: prepare-dpf-manifests
	...
	@for file in $(GENERATED_DIR)/*-crd.yaml $(GENERATED_DIR)/kamaji-crd.yaml $(GENERATED_DIR)/maintenance-oc-crd.yaml; do \
		if [ -f "$$file" ]; then \
			echo "Applying CRD $$file..."; \
			KUBECONFIG=$(KUBECONFIG_FILE) oc apply -f "$$file" || exit 1; \
		fi \
	done
	@echo "Waiting for CRDs to be established..."; \
	sleep 15

	# Apply SCC first to ensure permissions
	@if [ -f "$(GENERATED_DIR)/scc.yaml" ]; then \
		echo "Applying SCC..."; \
		KUBECONFIG=$(KUBECONFIG_FILE) oc apply -f "$(GENERATED_DIR)/scc.yaml" || exit 1; \
		sleep 5; \
	fi

	# Apply namespaces and basic resources
	...
	# Apply cert-manager and wait for readiness
	@if [ -f "$(GENERATED_DIR)/cert-manager-manifests.yaml" ]; then \
		echo "Applying cert-manager manifests..."; \
		KUBECONFIG=$(KUBECONFIG_FILE) oc apply -f "$(GENERATED_DIR)/cert-manager-manifests.yaml" || exit 1; \
		echo "Waiting for cert-manager webhook to be ready..."; \
		for i in {1..30}; do \
			if KUBECONFIG=$(KUBECONFIG_FILE) oc get pods -n cert-manager-operator -l app=webhook --field-selector status.phase=Running 2>/dev/null | grep -q "1/1"; then \
				echo "Cert-manager webhook is ready"; \
				break; \
			fi; \
			echo "Waiting for cert-manager webhook (attempt $$i/30)..."; \
			sleep 5; \
		done; \
		echo "Waiting additional time for cert-manager to stabilize..."; \
		sleep 5; \
	fi

	# Apply kamaji manifests first and wait for webhook
	@echo "Applying kamaji manifests..."
	@KUBECONFIG=$(KUBECONFIG_FILE) oc apply -f "$(GENERATED_DIR)/kamaji-manifests.yaml" || exit 1
	@echo "Waiting for kamaji webhook..."
	@for i in {1..15}; do \
		if KUBECONFIG=$(KUBECONFIG_FILE) oc get pods -n dpf-operator-system -l app=kamaji --field-selector status.phase=Running 2>/dev/null | grep -q "1/1"; then \
			echo "Kamaji webhook is ready"; \
			break; \
		fi; \
		if [ $$i -eq 15 ]; then \
			echo "Kamaji webhook failed to become ready"; \
			KUBECONFIG=$(KUBECONFIG_FILE) oc get pods -n dpf-operator-system; \
			KUBECONFIG=$(KUBECONFIG_FILE) oc describe pod -n dpf-operator-system -l app=kamaji; \
			exit 1; \
		fi; \
		echo "Waiting for kamaji webhook (attempt $$i/15)..."; \
		sleep 10; \
	done

	@echo "Waiting for etcd pods..."
	@for i in {1..30}; do \
		if KUBECONFIG=$(KUBECONFIG_FILE) oc get pods -n dpf-operator-system -l app=etcd --field-selector status.phase=Running 2>/dev/null | grep -q "3/3"; then \
			echo "Etcd pods are ready"; \
			break; \
		fi; \
		echo "Waiting for etcd pods (attempt $$i/30)..."; \
		sleep 10; \
	done
	...
	@for file in $(GENERATED_DIR)/*.yaml; do \
		if [[ ! "$$file" =~ .*(-ns)\.yaml$$ && \
			...
			if [[ "$$file" =~ .*operator.*\.yaml$$ ]]; then \
				echo "Waiting for operator resources..."; \
				sleep 10; \
			fi \
		fi \
	done
